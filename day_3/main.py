...
def highest_joltage(bank, num_cells):
    ...
    for highest_index, highest_battery_joltage in enumerate(sorted_bank):
        highest_location = bank.index(highest_battery_joltage)
        print(f"Trying battery with joltage of {highest_battery_joltage}")
        
        # Base case: only one cell needed
        if num_cells == 0:
            return bank[highest_location]
        
        # Check if we have enough batteries remaining
        if highest_location + num_cells >= len(bank):
            print("Not enough batteries left to get a full set")
            continue
        
        # Recursively find the next highest batteries
